  ...
  class EnvMetricV2ResponseBody
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::EnvMetricV2ResponseBody` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::EnvMetricV2ResponseBody`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'cdp')
        self.cdp = attributes[:'cdp']
      end
    ...
    def list_invalid_properties
      ...
      if @cdp.nil?
        invalid_properties.push('invalid value for "cdp", cdp cannot be nil.')
      end
      ...
      if @td.nil?
        invalid_properties.push('invalid value for "td", td cannot be nil.')
      end
    ...
    def hash
      [cdp, created_at, cve_id, role_id, role_name, td, updated_at].hash
    end
