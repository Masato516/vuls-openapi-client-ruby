...
module OpenapiClient
  class RoleGetRoleDetailResponseBody
    ...
    def self.openapi_types
      {
        ...
        :'servers' => :'Array<ServerChildResponseBody>',
        :'updated_at' => :'Time'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::RoleGetRoleDetailResponseBody` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::RoleGetRoleDetailResponseBody`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [all_task_count, created_at, env_metric_v2s, env_metric_v3s, id, is_default, name, new_task_count, sec_metric, servers, updated_at].hash
    end
